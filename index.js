document.addEventListener('DOMContentLoaded', () => {
    ...
    const endGame = (message) => {
        ...
        // Send match history to PHP script
        const matchData = {
            playerX: 'PlayerX',
            playerO: 'PlayerO',
            winner: message.includes('Draw') ? 'Draw' : currentPlayer
        };

        fetch('save_match_history.php', {
            method: 'POST',
            headers: {
...
// Function to fetch match history from the server
function fetchMatchHistory() {
    // Make an AJAX request to your PHP backend
    fetch('fetch_match_history.php')
        .then(response => response.json())
        .then(data => {
...
// Call the fetchMatchHistory function when the page loads
